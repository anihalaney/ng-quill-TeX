...
export default class QuillMathEditor extends Module {
    ...
    container: any[][];
    buttonIcon: string;
    range: QuillNamespace.RangeStatic;
    existingFomula: any;
    offset: any;
    ...
    constructor(quill, options) {
        ...
        if (mathEditors) {
            [].slice.call(mathEditors).forEach((mathsEditorBtn) => {
                mathsEditorBtn.innerHTML = this.buttonIcon;
            });
        }
    ...
    onClickLink(event, action, content, displayContent) {
